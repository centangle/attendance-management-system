...
<head>
	<title> View Assessment</title> <!-- Title of the Page -->
	...
	<?php
		...
		switch($error){
			case 1://message 1 case
				echo'<script type="text/javascript">alert("Error Occured Class Not Found.");</script>';//showing the alert box to notify the message to the user
				break;
			case 2://message 2 case
				echo'<script type="text/javascript">alert("Error Occured Teacher Not Found.");</script>';//showing the alert box to notify the message to the user
				break;
			case 5://message 5 case
				echo'<script type="text/javascript">alert("Success: Course Assesment Submitted.");</script>';//showing the alert box to notify the message to the user
				break;
		}
	?>
<!--Start of Body Tag -->
<body>	
	<?php include"studentheader.php"; ?> <!-- Side Bar Menu for Administrator -->
		<?php 				
			...
			?>
				...
				<h4 class="alert_success"><?php echo "Course Assessment is Closed."?></h4>
				</div>
			<?php
			}
				else
	...
	<div class="inlineSetting">
		<h4 class="alert_success">Course Name will be removed from list after assessment.</h4>
	</div>
					...
					<div class="da-panel-content">
						<table id="da-ex-datatable-numberpaging" class="da-table">
							...
							<tbody>
								...
								{
									$sub=$subjects[$i]['SubjectID'];
									$sql20=mysql_query("SELECT `SubjectID` FROM  `assesment` WHERE  `SubjectID`='$sub' LIMIT 1");
									//echo ("SELECT `SubjectID` FROM  `assesment` WHERE  `SubjectID`='$sub' LIMIT 1");
									$exist = mysql_fetch_row($sql20); 
									//echo $exist;
									//exit();
									if ($exist===false ) {
								?>
									<tr>
									   ...
									   <td>
										...
										<td>
											<a href="courseassesment.php?id=<?php echo $subjects[$i]['SubjectID'];?>&sID=<?php echo $subjects[$i]['SectionID'];?>">
