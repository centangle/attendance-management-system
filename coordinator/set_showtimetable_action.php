...
<?php
	...
	$b=checkPost($_POST, array('timeTabStatus'));
	if(!$b)
	{
	...
	$unsafe=$_POST['timeTabStatus']; //posting the user typr where notification will be sended
	$Status =clean($unsafe); //cleaning variable to prevent SQL injection
	
	$result8=mysql_query("SELECT *FROM `criteria` WHERE `Entity`='ShowTimetable' AND `Value`='$Status'" ); //checking either record already exists or not
	$exist1 = mysql_fetch_row($result8); //executing the query
    if ($exist1 !==false ) {
		if($Status=="Open")
		{
			redirect_to("criteria.php?ErrorID=7");//if already exists return with error code
		}
		else
			redirect_to("criteria.php?ErrorID=8");//if already exists return with error code
       }
	$sql=mysql_query("UPDATE `criteria` SET `Value`='$Status' WHERE `Entity`='ShowTimetable'");
	
	redirect_to("criteria.php?ErrorID=9");//redirecting toward register page
?>
<?php
}
else
