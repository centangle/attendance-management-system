...
<?php
	...
	$b=checkPost($_POST, array('lectureTitle','lecturDate','isLab','lectureStartTime','lectureEndTime','lectureDetails'));
	if(!$b)
	{
		redirect_to("updatelecturedetails.php?ErrorID=1&lid=$lectureID&sbID=$subjectID&scID=$sectionID");//retuning the error message back to the login page
		exit();//then exit
	}
	...
	$finalEndTime= $tempEndTime;
	
	$today=date('m/d/Y');
	if(strtotime($lecturDate)>strtotime($today))//if lecture date is greater than todays date show error
	{
		SetSessionValues();
		redirect_to("updatelecturedetails.php?ErrorID=6&lid=$lectureID&sbID=$subjectID&scID=$sectionID");
	}
	if(strtotime($finalStartTime)>=strtotime($finalEndTime))
	{
		SetSessionValues();
		redirect_to("updatelecturedetails.php?ErrorID=4&lid=$lectureID&sbID=$subjectID&scID=$sectionID"); //if file is greater than the given size or invalid extension
	}
	...
	{
		SetSessionValues();
		redirect_to("updatelecturedetails.php?ErrorID=7&lid=$lectureID&sbID=$subjectID&scID=$sectionID"); //redirecting toward add attendance page with error code
	}
	...
	if($isLab=="0")
	{
		...
		{
			SetSessionValues();
			redirect_to("updatelecturedetails.php?ErrorID=2&lid=$lectureID&sbID=$subjectID&scID=$sectionID"); //if file is greater than the given size or invalid extension
		}
	}
		else
			{
				...
				{
				SetSessionValues();
				redirect_to("updatelecturedetails.php?ErrorID=3&lid=$lectureID&sbID=$subjectID&scID=$sectionID"); //if file is greater than the given size or invalid extension
				}
			}
	...
	{
		mysql_query("UPDATE `lecture` 
					SET  `Title`='$lectureTitle',
						 `ClassID`='$classID',
						 `Description`='$lectureDetails',
						 `Date`='$lecturDate',
						 `Lab`='$isLab',
	...
	if($lectureContentFile!=NULL)
	{
		...
		if (	(
		  ...
		  {
			SetSessionValues();
			redirect_to("updatelecturedetails.php?ErrorID=5&lid=$lectureID&sbID=$subjectID&scID=$sectionID"); //if file is greater than the given size or invalid extension
		  }
	...
	$msg=clean("Updated Lecture Details in MSIS System");//action which is performed
	...
	redirect_to("lecturedetails.php?ErrorID=8&lid=$lectureID&sbID=$subjectID&scID=$sectionID");//redirectinf toward register page
?>
<?php
}
else
